...
RSpec.describe 'MultiBackgroundJob::Middleware::UniqueJob::Faktory', freeze_at: [2020, 7, 2, 12, 30, 50] do
  let(:worker_class) do
    Class.new do
      def self.name
        'DummyFaktoryWorker'
  ...
  shared_examples 'unique job across' do |across|
    subject(:push!) { worker.push(to: :faktory) }

    context "with a unique job across #{across}" do
      let(:lock_digest) do
        MultiBackgroundJob::LockDigest.new(:faktory, :mailer, across: across).to_s
      end
      ...
      it 'does not push job when active uniqueness lock exist' do
        redis do |conn|
          ...
          expect { |y| MultiBackgroundJob::Middleware::UniqueJob::Faktory::Worker.new.call(worker_class.new, job, &y) }.to yield_control
          expect(conn.zcount(lock_digest, 0, now + DAY_IN_SECONDS)).to eq(0)
        end
      end
      ...
      it 'adds a lock with 1 hour of TTL and enqueues a new job' do
        redis do |conn|
          ...
          Timecop.travel(travel_to) do
            new_result = worker.push(to: :faktory)
            ...
            # Removes the lock after job execution
            expect { |y| MultiBackgroundJob::Middleware::UniqueJob::Faktory::Worker.new.call(worker_class.new, result, &y) }.to yield_control
            expect(conn.zcount(lock_digest, 0, now + DAY_IN_SECONDS)).to eq(0)
          end
        end
      end
